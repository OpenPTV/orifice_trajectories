function res=draw_channel(id)


figure(id);

cL=[1 0 1];


%left_face
x5=[0 0      0      0];
y5=[0 0.0245 0.0245 0];
z5=[0 0      0.012  0.012];
patch(x5,y5,z5,cL)


%Right face
x6=[0.018 0.018 0.018 0.018];
y6=[0 0.0245 0.0245 0];
z6=[0 0 0.012 0.012];
patch(x6,y6,z6,cL)



% Bottom
x7=[0 0 0.018 0.018];
y7=[0 0.0245 0.0245 0];
z7=[0 0 0 0];
patch(x7,y7,z7,cL)




% Front face
x9=[0 0 0.018 0.018];
y9=[0 0 0 0];
z9=[0 0.012 0.012 0];
patch(x9,y9,z9,cL)


% Left face channel
x10=[0.0075 0.0075 0.0075 0.0075 0.0075];
y10=[0.0245 0.0245 0.045 0.045     0.0245];
z10=[0.009 0.012 0.012 0.009     0.009];
patch(x10,y10,z10,cL)
%plot3([0.0075 0.0075 ],[0.045 0.0245 ],[0.009 0.009 ],'k')
%channel bottom
x13=[0.0075 0.0075 0.0105 0.0105];
y13=[0.0245 0.045 0.045 0.0245];
z13=[0.009 0.009 0.009 0.009];
patch(x13,y13,z13,cL)



%plot3([0.0075 0.0075 ],[0.045 0.0245 ],[0.009 0.009 ],'k')


% Edge
x11=[0 0 0.0075 0.0075 0.0105 0.0105 0.018 0.018 ];
y11=[0.0245 0.0245 0.0245 0.0245 0.02450 0.0245 0.0245 0.0245 ];
z11=[0 0.012 0.012 0.009 0.009 0.012 0.012 0];
patch(x11,y11,z11,cL)


% Right face channel
x12=[ 0.0105 0.0105 0.0105 0.0105];
y12=[ 0.045 0.045 0.0245 0.0245];
z12=[0.009 0.012 0.012 0.009];
patch(x12,y12,z12,cL)
alpha(0.05)